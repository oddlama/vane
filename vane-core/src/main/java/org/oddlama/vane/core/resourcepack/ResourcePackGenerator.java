...
public class ResourcePackGenerator {
    ...
    public void add_item_model(NamespacedKey key, InputStream texture_png, InputStream texture_png_mcmeta, Key parent)
        throws IOException {
    ...
    private void write_item_models(final ZipOutputStream zip) throws IOException {
        for (var entry : item_textures.entrySet()) {
            ...
            if (texture_png_mcmeta.length > 0) {
                zip.putNextEntry(
                    new ZipEntry("assets/" + key.getNamespace() + "/textures/item/" + key.getKey() + ".png.mcmeta")
                );
