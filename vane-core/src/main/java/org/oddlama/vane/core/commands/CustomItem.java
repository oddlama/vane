...
public class CustomItem extends Command<Core> {
	public CustomItem(Context<Core> context) {
		...
		// Give custom item
		params().fixed("give").choice("custom_item",
				sender -> all_custom_items(),
				(sender, e) -> e.key().toString(),
				(sender, e) -> all_custom_items().stream().filter(i -> i.key().toString().equalsIgnoreCase(e)).findFirst().orElse(null))
	...
	private Collection<CustomItemVariant<?,?,?>> all_custom_items() {
		...
		get_module().core.for_all_module_components(c -> {
