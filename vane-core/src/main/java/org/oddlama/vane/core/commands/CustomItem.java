...
public class CustomItem extends Command<Core> {
	...
	public LiteralArgumentBuilder<CommandSourceStack> get_command_base() {
		// Help
		return super.get_command_base().executes(stack -> {
			print_help2(stack);
		...
		.then(literal("give")
			...
			.then(argument("custom_item", CustomItemArgumentType.customItem(get_module()))
			.executes(ctx -> {
			org.oddlama.vane.core.item.api.CustomItem item = ctx.getArgument("custom_item", org.oddlama.vane.core.item.api.CustomItem.class);
