...
public class Recipes<T extends Module<T>> extends ModuleComponent<T> {
	...
	public Recipes(final Context<T> context, final NamespacedKey base_recipe_key, final Supplier<RecipeList> def_recipes) {
		this(context, base_recipe_key, def_recipes, "The associated recipes. This is a map of recipe name to recipe definitions.");
	}

	public Recipes(final Context<T> context, final NamespacedKey base_recipe_key, final Supplier<RecipeList> def_recipes, final String desc) {
		super(context);
	...
	public void on_config_change() {
		...
		if (enabled() && config_register_recipes) {
			config_recipes.recipes().forEach(recipe -> get_module().getServer().addRecipe(recipe.to_recipe(base_recipe_key)));
		}
	}
