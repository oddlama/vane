...
public class CookingRecipeDefinition extends RecipeDefinition {
	...
	public CookingRecipeDefinition(String name, String type) {
		...
		switch (this.type) {
			...
			case "smoking":  break;
			default: throw new IllegalArgumentException("Invalid cooking recipe: Unknown type '" + this.type + "'");
		}
	}
	...
	public RecipeDefinition from_dict(Object dict) {
		if (!(dict instanceof Map<?,?>)) {
			throw new IllegalArgumentException("Invalid " + type + " recipe dictionary: Argument must be a Map<String, Object>!");
		}
	...
	public Recipe to_recipe(NamespacedKey base_key) {
		...
		switch (this.type) {
			...
			case "furnace":  return new FurnaceRecipe(key(base_key), out, in, experience, cooking_time);
			case "campfire": return new CampfireRecipe(key(base_key), out, in, experience, cooking_time);
			case "smoking":  return new SmokingRecipe(key(base_key), out, in, experience, cooking_time);
			default: throw new IllegalArgumentException("Invalid cooking recipe: Unknown type '" + this.type + "'");
		}
	}
}
