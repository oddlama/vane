...
public class RegionDynmapLayer extends ModuleComponent<Regions> {
	...
	@ConfigInt(def = 0x259df9, min = 0, desc = "Area marker fill color (0xRRGGBB).")
	...
	@ConfigInt(def = 0x259df9/*0xf9bd25*/, min = 0, desc = "Area marker line color (0xRRGGBB).")
	...
	public RegionDynmapLayer(final Context<Regions> context) {
		super(context.group("dynmap", "Enable dynmap integration. Regions will then be shown on a separate dynmap layer."));
	}
