	...
	public Config(File file) throws IOException {
		...
		for (final var multiplexer_conf : multiplexers_config.entrySet()) {
			...
			final var multiplexer = new AuthMultiplex(
					port,
					multiplexer_config.get("allowed_uuids")
			);
		...
		for (final var server_conf : servers_config.entrySet()) {
			...
			if (!(value instanceof final CommentedConfig managed_server_config)) throw new IllegalArgumentException("Managed server '" + key + "' has an invalid configuration!");
