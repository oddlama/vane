...
@SupportedAnnotationTypes("org.oddlama.imex.annotation.ConfigDouble")
@SupportedSourceVersion(SourceVersion.RELEASE_11)
public class ConfugAndLangProcessor extends AbstractProcessor {
	@Override
	...
	private void verifyIsClass(Element element) {
		if (element.getKind() != ElementKind.CLASS) {
			processingEnv.getMessager().printMessage(Diagnostic.Kind.ERROR, "@" + + " must be applied to a class");
		}
	}
	...
	private void verifyExtendsModule(Element element) {
		...
		if (!t.getSuperclass().toString().equals("org.oddlama.imex.core.Module")) {
			processingEnv.getMessager().printMessage(Diagnostic.Kind.ERROR, "@" + + " must be applied to a class inheriting from org.oddlama.imex.core.Module");
		}
	}
}
