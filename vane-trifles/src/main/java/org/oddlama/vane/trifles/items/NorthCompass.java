...
public class NorthCompass extends CustomItem<Trifles> {
    ...
    public void addResources(final ResourcePackGenerator rp) throws IOException {
        ...
        for (final var angle_item : angle_item_overrides) {
            ...
            final var key_num = num == 16
                ? base_key
                : StorageUtil.namespaced_key(base_key.namespace(), String.format("%s_%02d", base_key.value(), num));
        ...
        for (final var angle_item : angle_item_overrides) {
            ...
            final var key_num = StorageUtil.namespaced_key(
                key().namespace(),
                String.format("%s_%02d", key().value(), num)
            );
